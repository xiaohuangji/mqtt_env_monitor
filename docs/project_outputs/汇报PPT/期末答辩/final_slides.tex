...
\begin{frame}{Broker 选型：EMQX 单机扛万级（实测）}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.60\textwidth}
      \centering\includegraphics[width=\linewidth, height=0.66\textheight, keepaspectratio]{images/exp_capacity.png}
    \end{column}
  \end{columns}
...
\begin{frame}{实测佐证 · 吞吐}
  ...
  \begin{columns}[c]
    \begin{column}{0.64\textwidth}
      \centering\includegraphics[width=\linewidth, height=0.66\textheight, keepaspectratio]{images/exp_throughput.png}
    \end{column}
...
\begin{frame}{实测佐证 · QoS × 丢包}
  ...
  \begin{columns}[c]
    \begin{column}{0.34\textwidth}
      \bignum{100\%}{0–15\% 丢包下\\ 三种 QoS 到达率\\ 均 100\%（TCP 重传）}
    \end{column}
    \begin{column}{0.64\textwidth}
      \centering\includegraphics[width=\linewidth, height=0.66\textheight, keepaspectratio]{images/exp_qos_loss.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{ESP32-C3 硬件节点：组成与配网}
  \vspace{4pt}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \textbf{\footnotesize 传感器接入（模块化、可扩展）}\\[4pt]
      {\scriptsize
      \begin{tabular}{lll}
        \toprule
        传感器 & 接口 & 采集量 \\
    ...
    \begin{column}{0.46\textwidth}
      \textbf{\footnotesize Wi-Fi 配网（AP + WiFiManager）}
      {\footnotesize
      \begin{itemize}\itemsep=4pt
        \item 不写死 Wi-Fi；首次或失败进入配网热点
        \item 手机/PC 打开网页，选择 Wi-Fi 并保存
        \item 换实验室或热点免改代码、免重烧录
      \end{itemize}}
    \end{column}
  \end{columns}
  \vfill
  \begin{center}\small\color{black!55}
    每类传感器独立采集函数、主程序统一调度——新增传感器不动主结构。
  \end{center}
\end{frame}

\begin{frame}{硬件节点：运行流程}
  \vspace{14pt}
  \begin{center}
    \begin{tikzpicture}[
        b/.style={draw=black!50, rounded corners=2pt, fill=black!4, align=center,
                  font=\scriptsize, minimum height=0.9cm, minimum width=2.3cm, inner sep=3pt},
        hub/.style={b, fill=mLightBrown!12, draw=mLightBrown},
        a/.style={-{Stealth[length=2mm]}, black!55, semithick}]
      \node[b] (s1) {启动};
      \node[b, right=0.45cm of s1] (s2) {初始化\\\scriptsize 串口/传感器/配网};
      \node[b, right=0.45cm of s2] (s3) {连接 Wi-Fi};
      \node[b, right=0.45cm of s3] (s4) {NTP 时间同步};
      \node[hub, below=0.8cm of s4] (s5) {连接 MQTT};
      \node[b, left=0.45cm of s5] (s6) {周期采集 30 s};
      \node[b, left=0.45cm of s6] (s7) {JSON 封装};
      \node[b, left=0.45cm of s7] (s8) {发布上传};
      \draw[a] (s1)--(s2); \draw[a] (s2)--(s3); \draw[a] (s3)--(s4); \draw[a] (s4)--(s5);
      \draw[a] (s5)--(s6); \draw[a] (s6)--(s7); \draw[a] (s7)--(s8);
    \end{tikzpicture}
  \end{center}
  \takeaway{仅当 Wi-Fi 与 NTP 时间同步就绪才上传带时间戳数据；断网或 MQTT 断开自动重连后续传}
\end{frame}
...
\begin{frame}{光照实测与硬件工程改进}
  ...
  \begin{columns}[c]
    \begin{column}{0.61\textwidth}
      \centering\includegraphics[width=\linewidth, height=0.50\textheight, keepaspectratio]{images/light_dawn.png}
    \end{column}
