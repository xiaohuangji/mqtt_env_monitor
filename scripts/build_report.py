...
body("为给上述判断提供量化依据，本系统用同一条环境数据分别经 MQTT（QoS 1 发布）与 HTTP（POST 上传）发送，逐字节实测应用层开销（脚本见 scripts/compare_mqtt_http.py，每种格式各取 200 次）。由于应用层报文字节数与对端在本地或公网无关，该对比在本地即可精确复现。结果如表 4-2：MQTT 每条仅约 40 字节协议头，而 HTTP 每条都要重发完整请求头（约 340 字节，约为 MQTT 的 8 倍）；上传同一条数据，MQTT 比 HTTP 节省 65 %–77 %，且 payload 越小、上传越频繁，HTTP 的固定大头占比越高、MQTT 越省——这正契合多节点周期上传小数据的本场景。此外，HTTP 若不保持长连接，每次上传还需重做 TCP 三次握手，而 MQTT 一次连接长期复用、仅在建链时有约 28 字节开销。")
table(["payload 格式", "MQTT 单次（B）", "HTTP 单次（B）", "MQTT 节省"],
      [["json（122 B）", "163", "463", "65 %"],
       ["json-min（65 B）", "105", "405", "74 %"],
       ["msgpack（49 B）", "89", "392", "77 %"]],
      cap="表 4-2  MQTT 与 HTTP 单次上传应用层开销实测对比（N=200，含报文头）")
h2("4.2  主题结构与消息格式设计")
...
h1("5  实现过程与关键配置")
h2("5.1  PC 模拟节点")
body("PC 模拟节点（src/simulated_nodes/sim_node.py）以均值回归随机游走生成平滑且贴近真实环境的数据，避免数值突跳。节点编号、数据类型、上传周期、QoS、消息格式与协议版本均可通过命令行配置，例如 python sim_node.py --node-id node01 --types temperature,humidity --interval 5 --qos 1 --format json-min；其默认发布的数据类型与 ESP32 节点对齐，载荷字段也完全一致，因而二者可同构接入同一 Broker。在 MQTT 5.0 下按连接维护 Topic Alias，断线时 QoS 1 消息在本地排队并于重连后补发。模拟节点既用于在硬件就绪前打通链路，也用于规模化压测（配合 emqtt_bench 可达数万连接）。")
h2("5.2  ESP32 硬件节点")
body("ESP32 硬件节点基于 ESP32-C3-32S 开发板，连接 DHT11（温湿度）、BH1750（光照，I2C）、MQ-2（可燃气体）、FC-28（土壤湿度）四个传感器模块，引脚分配如表 5-1，模块实物与整体接线分别见图 5-1、图 5-2。每类传感器对应一个独立的采集模块（dht11 / light / mq2 / soil_sensor），由主程序统一调度，新增传感器不影响主结构；固件以 PlatformIO 组织（hardware/esp32_nodes/ESP-C3-32S）。")
table(["传感器", "型号", "接口 / 引脚", "采集量"],
      [["温湿度", "DHT11", "GPIO5（单总线）", "温度 · 湿度"],
       ["光照", "BH1750", "I2C（SDA=GPIO1 / SCL=GPIO2）", "光照（lux）"],
       ["土壤湿度", "FC-28", "GPIO3（ADC）", "土壤湿度（%）"],
       ["可燃气体", "MQ-2", "GPIO4（ADC）", "可燃气体（原始值）"]],
      cap="表 5-1  ESP32 硬件节点传感器接入")
figure(os.path.join(PPTIMG, "hw_sensors.png"), "图 5-1  各传感器模块实物", 12.0)
figure(os.path.join(PPTIMG, "hw_wiring.png"), "图 5-2  开发板与传感器整体接线图", 13.5)
body("节点上电后的工作流程如图 5-3：①初始化串口、各传感器与 Wi-Fi 配网模块；②经 WiFiManager 连接 Wi-Fi（首次或连接失败时进入配网热点，见本节后段）；③Wi-Fi 就绪后进行 NTP 时间同步，使每条数据携带准确的北京时间戳（epoch 毫秒）；④以 MQTT 3.1.1、QoS 1、匿名方式连接 Broker；⑤按约 30 秒周期循环采集五类数据；⑥封装为紧凑的 json-min 载荷（短键 n / d / v / t / s）；⑦按主题 env_monitor/{node_id}/{data_type} 逐类发布；⑧串口输出运行日志；⑨网络或 MQTT 断开时自动重连并续传。节点由充电宝供电以支持室外长时间运行，其接口与 PC 模拟节点完全一致，可直接接入既有链路而无需改动监控端。")
figure(os.path.join(PPTIMG, "hw_flow.png"), "图 5-3  ESP32 硬件节点程序流程", 7.0)
body("图 5-4 为节点运行时的串口日志：可见各传感器初始化与引脚映射、NTP 对时成功（北京时间与 epoch 毫秒）、MQTT 以 QoS 1 连接成功，以及按“温度→湿度→光照→可燃气体→土壤湿度”顺序的周期发布过程，与上述流程一致。")
figure(os.path.join(PPTIMG, "hw_serial.png"), "图 5-4  ESP32 节点串口运行日志", 12.5)
body("Wi-Fi 配网采用 AP + WiFiManager 方案：节点不写死 Wi-Fi 账号密码，首次启动或连接失败时自动开启名为 ESP32C3_Config 的配网热点，用户用手机或电脑连接后访问 192.168.4.1 的配网门户（图 5-5），在网页上选择目标 Wi-Fi 并保存，节点随即连接并在成功后输出 IP、网关与 RSSI（图 5-6 中可见 ESP32 已接入热点、获得地址 192.168.137.81）。该机制解决了固定 Wi-Fi 不便维护的问题，使节点具备换网配置、自动连接与断线恢复能力，更换实验室或热点时免改代码、免重新烧录。")
figure(os.path.join(PPTIMG, "hw_portal1.jpg"), "图 5-5  ESP32 Wi-Fi 配网门户页面", 9.0)
figure(os.path.join(PPTIMG, "hw_wifi_ok.png"), "图 5-6  节点成功接入热点并获得 IP 地址", 12.0)
h2("5.3  Web 监控端")
body("Web 监控端（src/monitor/web_monitor.py）基于 Flask 提供页面，后端用 paho-mqtt 订阅 env_monitor/+/+，并通过服务器推送事件（SSE）将更新实时推送到浏览器，页面无需刷新即可更新各节点的最新数值与在线状态；超过 15 秒未收到数据的条目自动标记为离线/过期。页面还提供“选择订阅”控件，可按节点（env_monitor/{node}/+）、按数据类型（env_monitor/+/{type}）或全部进行订阅过滤，后端按 MQTT 通配符对该连接的 SSE 推送做相应过滤，直观体现 MQTT 的主题订阅机制。监控端同时记录毫秒级接收日志（含 seq、QoS、格式、原始载荷），为到达率与延迟统计提供证据；并提供 /calculator 路由承载需求-成本计算器。监控端实时界面见图 5-7。")
figure(os.path.join(MIDIMG, "web_monitor_online_crop.png"), "图 5-7  Web 监控端实时显示界面（双节点并发）", 14.0)
h2("5.4  EMQX 部署与系统调优")
...
h2("6.9  典型问题与解决过程")
body("课程设计过程中遇到若干典型问题，下面按“问题现象—原因分析—修改措施—复测结果”整理为表 6-5，以体现调试与持续改进过程。")
table(["问题", "现象", "原因分析", "修改措施", "复测结果"],
      [["BH1750 光照异常", "白天约 99 % 读数为 -1，夜间能连续读数十分钟后又掉", "ESP32 端 I2C 接线接触不良、白天受热接触更差（非模块损坏、非订阅侧）", "重新整理共地与接线、加入数据有效性判断", "6-16 凌晨至清晨连续 6 h 全部有效（0→305 lux），白天约 8000 lux"],
       ["WiFiManager 配网", "配网门户页面偶发访问不稳定", "AP 门户实现与 DNS / 路由处理不完善", "优化配网网页门户与热点重连逻辑", "配网页面访问稳定，现场部署可靠"],
       ["MQTT 断线", "热点波动导致节点掉线", "无线链路不稳定", "启用 keepalive 心跳 + 自动重连", "断线后约 5–7 s 自动重连续传"],
       ["离线丢数据", "断线期间发布的数据可能丢失", "QoS 0 无确认、无暂存", "改用 QoS 1 + 持久会话，监控端按 seq 去重", "离线期间 12 条全部补发、零丢失"]],
      cap="表 6-5  典型问题与解决过程")
h2("6.10  设计指标达成情况汇总")
body("将第 2 章定义的设计指标与本章实测结果逐项对照，汇总于表 6-6，各项均达标。")
table(["设计指标", "目标值", "实测值", "是否达标", "证据来源"],
      [["消息到达率", "≥ 95 %", "功能测试 100 %；ESP32 真机 15 h 为 95.0 %", "达标", "表 6-1、6.7 节"],
       ["平均传输延迟", "≤ 1 s", "跨公网平均 44.7 ms", "达标", "图 6-6"],
       ["断线重连恢复", "≤ 10 s", "约 5.2 ~ 7 s", "达标", "表 6-1、6.5 节"],
       ["连续运行稳定性", "≥ 30 min", "压测 30 min 零掉线；真机约 15 h", "达标", "图 6-7、6.7 节"],
       ["并发节点数", "≥ 2（验证万级）", "双节点功能 + 2 万连接压测", "达标", "表 6-1、图 6-7"]],
      cap="表 6-6  设计指标达成情况汇总")
...
h2("7.2  硬件节点工程约束")
bullets([
    "传感器精度：DHT11 为消费级（温度 ±2 ℃、湿度 ±5 %），MQ-2、FC-28 输出为未标定原始值，仅反映变化趋势；精确计量需选用更高精度传感器并做标定。",
    "供电方式：现用充电宝供电、续航有限；室外长期部署需太阳能+锂电或市电，并配合低功耗休眠延长续航。",
    "室外环境：高温、潮湿、灰尘与日晒会影响读数与寿命，户外部署需防水防尘外壳与遮阳。",
    "线材与接触：BH1750 早期因接线接触不良导致间歇缺失，户外振动/温变下接触可靠性尤为关键，应采用焊接或锁紧端子并做应力释放。",
    "Wi-Fi 覆盖：万级节点依赖校园 Wi-Fi 覆盖与 AP 关联容量（每 AP 约 200 个终端），盲区需增设 AP 或改用 LoRa / NB-IoT 等广域低功耗方案。",
    "节点维护：节点量大时，固件批量升级（OTA）、故障定位与现场更换的维护成本不可忽视，需配套设备管理与告警。",
])
h2("7.3  存在的不足")
bullets([
...
h2("7.5  安全性现状与改进")
body("当前阶段，本系统主要在局域网与受控测试环境中验证 MQTT 通信链路的功能与性能，Broker 采用匿名连接、未启用认证与加密：一方面把工作重心放在功能与容量/性能测试上，另一方面测试网络封闭、接入设备可控、安全风险较低。但匿名开放的 Broker 存在被非法节点伪造发布或外部越权订阅敏感数据的风险，因此在实际对外部署时，应启用用户名口令/客户端证书认证、基于主题的 ACL 访问控制以及 TLS 加密传输，并结合网关或防火墙限制接入来源，以防止非法发布与越权订阅。")
...
body("在工程实践与协作方面，本小组采用软硬件分工协同：软件侧完成协议升级、网络负载与成本建模、需求-成本计算器、监控端与压测，硬件侧完成 ESP32 传感器接入、固件开发与 Wi-Fi 配网；全过程基于 Git 进行分支开发、Issue 跟踪与 Pull Request 评审，保留了从原始设计到修改方案的完整过程材料与测试证据（抓包文件、压测数据、串口与运行日志、真机曲线）。回顾整个设计，从“能跑通”到“以需求驱动、量化论证最低成本”，再到真机长时间验证与典型问题的闭环修复，较完整地经历了一次面向工程需求的网络系统设计：明确需求与指标、比选并设计方案、实现与调优、实验验证、约束分析与改进。后续可在硬件可靠性、安全加固、数据持久化与超限告警等方向继续完善，向可实际部署的系统演进。")
h2("8.2  参考资料")
refs = [
